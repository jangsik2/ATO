...
let AtoService = class AtoService {
    ...
    async test(params) {
        ...
        const dbRes = await this.mysql.connectionPool.execute(`
    ...
    async fmailyTranslate(data, language, documentId, Dtype, reload) {
        console.log(data);
        let defaultData = {
        ...
        try {
            var type = "";
            if (data.documentTitle.includes("상"))
                type = "(상세)";
            else if (data.documentTitle.includes("출"))
                type = "(특정- 출생 ∙ 사망 ∙ 실종)";
            else if (data.documentTitle.includes("친"))
                type = "(특정 - 친권 ∙ 후견)";
            else if (data.documentTitle.includes("일"))
                type = "(일반)";
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["가족관계증명서", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                data.documentTitle = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [type, "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                data.documentType = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["등록기준지", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.addressTitle = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["구분", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.category = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["상세내용", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.details = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["성명", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.name = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["출생연월일", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.dateOfBirth = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["주민등록번호", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.residentNO = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["성별", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.gender = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["본", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.surnameOrigin = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["가족사항", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.RPR = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["사망", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.deceased = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["국적상실", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.lost = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["국적", "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
        ...
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "본인" and document = "가족관계증명서"`);
        result = result[0];
        if (result.length > 0)
        ...
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [data.principal.gender, "가족관계증명서"]);
        result = result[0];
        if (result.length > 0)
            data.principal.gender = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_surnameOrigin where cn = ?`, [data.principal.surnameOrigin]);
        result = result[0];
        if (result.length > 0)
            data.principal.surnameOrigin = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "부" and document = "가족관계증명서"`);
        result = result[0];
        if (result.length > 0)
        ...
        if (data.father.status != 0) {
            ...
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.father.gender}" and document = "가족관계증명서"`);
            result = result[0];
            if (result.length > 0)
                data.father.gender = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_surnameOrigin where cn = "${data.father.surnameOrigin}"`);
            result = result[0];
            if (result.length > 0)
                data.father.surnameOrigin = result[0][language];
        }
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "모" and document = "가족관계증명서"`);
        result = result[0];
        if (result.length > 0)
        ...
        if (data.mother.status != 0) {
            ...
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.mother.gender}" and document = "가족관계증명서"`);
            result = result[0];
            if (result.length > 0)
                data.mother.gender = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_surnameOrigin where cn = "${data.mother.surnameOrigin}"`);
            result = result[0];
            if (result.length > 0)
                data.mother.surnameOrigin = result[0][language];
        }
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["배우자", "가족관계증명서"]);
        result = result[0];
        if (result.length > 0)
        ...
        if (data.spouse.status != 0) {
            ...
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [data.spouse.gender, "가족관계증명서"]);
            result = result[0];
            if (result.length > 0)
                data.spouse.gender = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_surnameOrigin where cn = ?`, [data.spouse.surnameOrigin]);
            result = result[0];
            if (result.length > 0)
                data.spouse.surnameOrigin = result[0][language];
        }
        for (var i = 0; i < data.child.length; i++) {
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "자녀" and document = "가족관계증명서"`);
            result = result[0];
            if (result.length > 0)
            ...
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.child[i].gender}" and document = "가족관계증명서"`);
            result = result[0];
            if (result.length > 0)
                data.child[i].gender = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_surnameOrigin where cn = "${data.child[i].surnameOrigin}"`);
            result = result[0];
            if (result.length > 0)
        ...
        if (data.detail !== null) {
            ...
            for (var i = 0; i < data.detail.length; i++) {
                var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [data.detail[i].category, "가족관계증명서"]);
                result = result[0];
                if (result.length > 0)
                    data.detail[i].category = result[0][language];
                for (var j = 0; j < data.detail[i].content.length; j++) {
                    var result = await this.mysql.connectionPool.execute(`SELECT ${language}, input FROM test_GIGAHON where ko like ? and document = ?`, [data.detail[i].content[j].split(' ')[0] + "%", "가족관계증명서"]);
                    result = result[0];
                    if (result.length > 0) {
                        var location = 0;
                        ...
                        if (result[location].input.includes("날짜")) {
                            ...
                            if ((result[location][language].split("이름").length - 1) === 0 && (result[location][language].split("성명").length - 1) === 0) {
                                ...
                                data.detail[i].content[j] = result[location][language];
                        }
        ...
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.text1}" and document = "가족관계증명서"`);
        result = result[0];
        if (result.length > 0)
        ...
        if (data.closure != "") {
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "[폐쇄]" and document = "가족관계증명서"`);
            result = result[0];
            if (result.length > 0)
                data.closure = result[0][language];
        }
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ?`, [data.diplomaticMA]);
        result = result[0];
        if (result.length > 0)
            data.diplomaticMA = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["발급담당자", "가족관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.titleIssuer = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["☏", "가족관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.titleNumber = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["발급시각", "가족관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.titleTimeIssue = result[0][language];
        if (!reload)
            data.timeIssue = data.timeIssue.substr(0, 2) + ":" + data.timeIssue.substr(4, 2);
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["신청인", "가족관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.titleApplicant = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ?`, [data.text2]);
        result = result[0];
        if (result.length > 0)
        ...
        if (data.agencyIssue.includes("무인증명서")) {
            ...
            var result = await this.mysql.connectionPool.execute(`SELECT spelling FROM namespelling where name = "${agencyName[0]}"`);
            result = result[0];
            data.agencyIssue = result[0].spelling.split("|")[0];
            var result = await this.mysql.connectionPool.execute(`SELECT spelling FROM namespelling where name = "${agencyName[1]}"`);
            result = result[0];
            data.agencyIssue += ", " + result[0].spelling.split("|")[0];
            var result = await this.mysql.connectionPool.execute(`SELECT spelling FROM namespelling where name = "${agencyName[2]}"`);
            result = result[0];
            data.agencyIssue += result[0].spelling.split("|")[0];
  ...
  Judicial Information Technology Center, National Court Administration`;
            ...
            result = result[0];
            data.agencyIssue += result[0].spelling.split("|")[0];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_city where ko = "${place}"`);
            result = result[0];
            if (result.length > 0) {
                data.agencyIssue += ", Head of " + result[0][language];
                data.footerAgency = "Head of " + result[0][language];
            }
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "(수입증지가 인영이 되지 아니한 증명은 그 효력을 보증할 수 없습니다.)" and document = "가족관계증명서"`);
            result = result[0];
            if (result.length > 0)
        ...
        var status = 0;
        var result = await this.mysql.connectionPool.execute(`SELECT d_status FROM documentType where d_title = ?`, [data.documentTitle]);
        result = result[0];
        if (result.length > 0)
    ...
    async basicDocument(fullText) {
        ...
        for (var i = 0; i < fullText.length; i++) {
            ...
            if ((fullText[i][5] == "상 세 내 용" || fullText[i][5] == "상세 내 용" || fullText[i][5] == "상 세내 용" || fullText[i][5] == "상 세 내용" || fullText[i][5] == "상 세내용") && detailCount == 0 && info.principal.name == "") {
                var categoryCount = 0;
                var contentCount = 0;
                var detailCategoryBBOX = 0;
            ...
            else if ((fullText[i][5] == "상 세 내 용" || fullText[i][5] == "상세 내 용" || fullText[i][5] == "상 세내 용" || fullText[i][5] == "상 세 내용" || fullText[i][5] == "상 세내용") && info.detailUp[0].category != 0) {
                if (detailCount == 0)
                    info.detailUp = null;
                var categoryCount = 0;
                var contentCount = 0;
    ...
    async basicTranslate(data, language, documentId, Dtype, reload) {
        console.log(data);
        let defaultData = {
        ...
        try {
            ...
            if (data.documentTitle.includes("상"))
                ...
                type = "(일반)";
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "기본증명서" and document = "기본증명서"`);
            result = result[0];
            if (result.length > 0)
                data.documentTitle = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${type}" and document = "기본증명서"`);
            result = result[0];
            console.log(result);
            if (result.length > 0)
                data.documentType = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["등록기준지", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.addressTitle = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["구분", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.category = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["상세내용", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.details = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["성명", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.name = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["출생연월일", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.dateOfBirth = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["주민등록번호", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.residentNO = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["성별", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.gender = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["본", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.surnameOrigin = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["일반등록사항", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.RPR = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["사망", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.deceased = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["국적상실", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.lost = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["국적", "기본증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.nationality = result[0][language];
            try {
        ...
        if (data.detailUp !== null) {
            ...
            for (var i = 0; i < data.detailUp.length; i++) {
                var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.detailUp[i].category}" and document = "기본증명서"`);
                result = result[0];
                if (result.length > 0)
                    data.detailUp[i].category = result[0][language];
                for (var j = 0; j < data.detailUp[i].content.length; j++) {
                    var result = await this.mysql.connectionPool.execute(`SELECT ${language}, input FROM test_GIGAHON where ko like "${data.detailUp[i].content[j].split(' ')[0]}%" and document = "기본증명서"`);
                    result = result[0];
                    if (result.length > 0) {
                        var location = 0;
                        var CheckLevensData = 0;
                        ...
                        if (result[location].input.includes("날짜")) {
                            ...
                            if ((result[location][language].split("이름").length - 1) === 0 && (result[location][language].split("성명").length - 1) === 0) {
                                ...
                                data.detailUp[i].content[j] = result[location][language];
                        }
                        else if (result[location].input.includes("주민번호")) {
                            if ((result[location][language].split("주민번호").length - 1) === 0) {
        ...
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "본인" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            defaultData.principalTitle = result[0][language];
        if (data.principal.birth.length < 15 && data.principal.birth.charCodeAt(0) > 91 || 64 > data.principal.birth[0].charCodeAt(0))
            ...
            data.principal.birth = date[Number(data.principal.birth.substr(6, 2)) - 1] + data.principal.birth.substr(10, 2) + ", " + data.principal.birth.substr(0, 4) + " " + data.principal.birth.substr(14, 2) + ":" + data.principal.birth.substr(18, 2) + ":" + data.principal.birth.substr(22, 2);
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.principal.gender}" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            data.principal.gender = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.principal.surnameOrigin}" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            data.principal.surnameOrigin = result[0][language];
        if (data.detailDown !== null) {
            ...
            for (var i = 0; i < data.detailDown.length; i++) {
                var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.detailDown[i].category}" and document = "기본증명서"`);
                result = result[0];
                if (result.length > 0)
                    data.detailDown[i].category = result[0][language];
                for (var j = 0; j < data.detailDown[i].content.length; j++) {
                    var result = await this.mysql.connectionPool.execute(`SELECT ${language}, input FROM test_GIGAHON where ko like "${data.detailDown[i].content[j].split(' ')[0]}%" and document = "기본증명서"`);
                    result = result[0];
                    if (result.length > 0) {
                        var location = 0;
                        var CheckLevensData = 0;
                        ...
                        if (result[location].input.includes("날짜")) {
                            ...
                            if ((result[location][language].split("이름").length - 1) === 0 && (result[location][language].split("성명").length - 1) === 0) {
                                ...
                                data.detailDown[i].content[j] = result[location][language];
                        }
                        else if (result[location].input.includes("주민번호")) {
                            if ((result[location][language].split("주민번호").length - 1) === 0) {
        ...
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.text1}" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            data.text1 = result[0][language];
        if (!reload)
            data.dateIssue = date[Number(data.dateIssue.substr(6, 2)) - 1] + data.dateIssue.substr(10, 2) + ", " + data.dateIssue.substr(0, 4);
        if (data.closure != "") {
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "[폐쇄]" and document = "기본증명서"`);
            result = result[0];
            if (result.length > 0)
                data.closure = result[0][language];
        }
        ...
        if (result.length > 0)
            data.diplomaticMA = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "발급담당자" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            defaultData.titleIssuer = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "☏" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            defaultData.titleNumber = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "발급시각" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            defaultData.titleTimeIssue = result[0][language];
        data.timeIssue = data.timeIssue.substr(0, 2) + ":" + data.timeIssue.substr(4, 2);
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "신청인" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            defaultData.titleApplicant = result[0][language];
        data.timeIssue = data.timeIssue.substr(0, 2) + ":" + data.timeIssue.substr(4, 2);
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = "${data.text2}" and document = "기본증명서"`);
        result = result[0];
        if (result.length > 0)
            data.text2 = result[0][language];
        if (data.agencyIssue.includes("무인증명서")) {
  ...
  Judicial Information Technology Center National Court Administration`;
    ...
    async marriagageTranslate(data, language, documentId, Dtype, reload) {
        let defaultData = {
            address: data.address,
            addressTitle: "", category: "",
            details: [{
        ...
        try {
            ...
            if (data.documentTitle.includes("상"))
                ...
                type = "(일반)";
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["혼인관계증명서", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                data.documentTitle = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [type, "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                data.documentType = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["등록기준지", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.addressTitle = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["구분", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.category = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["상세내용", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.details = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["성명", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.name = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["출생연월일", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.dateOfBirth = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["주민등록번호", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.residentNO = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["성별", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.gender = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["본", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.surnameOrigin = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["혼인사항", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.RPR = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["사망", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.deceased = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["국적상실", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.lost = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["국적", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                defaultData.nationality = result[0][language];
            try {
        ...
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["본인", "혼인관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.principalTitle = result[0][language];
        if (data.principal.birth.length < 15 && data.principal.birth.charCodeAt(0) > 91 || 64 > data.principal.birth[0].charCodeAt(0))
            ...
            data.principal.birth = date[Number(data.principal.birth.substr(6, 2)) - 1] + data.principal.birth.substr(10, 2) + ", " + data.principal.birth.substr(0, 4) + " " + data.principal.birth.substr(14, 2) + ":" + data.principal.birth.substr(18, 2) + ":" + data.principal.birth.substr(22, 2);
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [data.principal.gender, "혼인관계증명서"]);
        result = result[0];
        if (result.length > 0)
            data.principal.gender = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_surnameOrigin where cn = ?`, [data.principal.surnameOrigin]);
        ...
        if (result.length > 0)
            data.principal.surnameOrigin = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["배우자", "혼인관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.spouseTitle = result[0][language];
        if (data.spouse.status != 0) {
            if (data.spouse.birth.length < 15 && data.spouse.birth.charCodeAt(0) > 91 || 64 > data.spouse.birth[0].charCodeAt(0))
                ...
                data.spouse.birth = date[Number(data.spouse.birth.substr(6, 2)) - 1] + data.spouse.birth.substr(10, 2) + ", " + data.spouse.birth.substr(0, 4) + " " + data.spouse.birth.substr(14, 2) + ":" + data.Spouse.birth.substr(18, 2) + ":" + data.Spouse.birth.substr(22, 2);
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [data.spouse.gender, "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                data.spouse.gender = result[0][language];
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_surnameOrigin where cn = ?`, [data.spouse.surnameOrigin]);
        ...
        if (data.detail !== null) {
            if (!reload)
                data.detail.length = data.detail.length - 1;
            for (var i = 0; i < data.detail.length; i++) {
                var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [data.detail[i].category, "혼인관계증명서"]);
                result = result[0];
                if (result.length > 0)
                    data.detail[i].category = result[0][language];
                for (var j = 0; j < data.detail[i].content.length; j++) {
                    var result = await this.mysql.connectionPool.execute(`SELECT ${language}, input FROM test_GIGAHON where ko like ? and document = ?`, [data.detail[i].content[j].split(' ')[0] + "%", "혼인관계증명서"]);
                    result = result[0];
                    if (result.length > 0) {
                        var location = 0;
                        var CheckLevensData = 0;
        ...
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, [data.text1, "혼인관계증명서"]);
        result = result[0];
        if (result.length > 0)
            data.text1 = result[0][language];
        if (!reload)
            data.dateIssue = date[Number(data.dateIssue.substr(6, 2)) - 1] + data.dateIssue.substr(10, 2) + ", " + data.dateIssue.substr(0, 4);
        if (data.closure != "") {
            var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["[폐쇄]", "혼인관계증명서"]);
            result = result[0];
            if (result.length > 0)
                data.closure = result[0][language];
        }
        ...
        if (result.length > 0)
            data.diplomaticMA = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["발급담당자", "혼인관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.titleIssuer = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["☏", "혼인관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.titleNumber = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["발급시각", "혼인관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.titleTimeIssue = result[0][language];
        if (!reload)
            data.timeIssue = data.timeIssue.substr(0, 2) + ":" + data.timeIssue.substr(4, 2);
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ? and document = ?`, ["신청인", "혼인관계증명서"]);
        result = result[0];
        if (result.length > 0)
            defaultData.titleApplicant = result[0][language];
        var result = await this.mysql.connectionPool.execute(`SELECT ${language} FROM test_GIGAHON where ko = ?`, [data.text2]);
  ...
  Judicial Information Technology Center National Court Administration`;
    ...
    async familyMakeWordDocument(data) {
        ...
        const content = fs.readFileSync(path.resolve("./wordData", `가족_${data.input.type}.docx`), "binary");
    ...
    async basicMakeWordDocument(data) {
        ...
        const fs = require("fs");
        const path = require("path");
        const content = fs.readFileSync(path.resolve("./wordData", `기본_${data.input.type}.docx`), "binary");
        const zip = new PizZip(content);
        const doc = new Docxtemplater(zip, {
            paragraphLoop: true,
            linebreaks: true,
    ...
    async marriagageMakeWordDocument(data) {
        ...
        const fs = require("fs");
        const path = require("path");
        const content = fs.readFileSync(path.resolve("./wordData", `혼인_${data.input.type}.docx`), "binary");
        const zip = new PizZip(content);
        const doc = new Docxtemplater(zip, {
            paragraphLoop: true,
            linebreaks: true,
    ...
    async translateName(input) {
        ...
        for (var i = 0; i < input.length; i++) {
            var result = await this.mysql.connectionPool.execute("SELECT spelling FROM namespelling where name = ?", [input[i]]);
            result = result[0];
        ...
        for (let i = 0; i < splitedTargetString.length; i++) {
            ...
            if (city.filter((el) => { return splitedTargetString[i].includes(el); }).length > 0) {
                ...
                for (let j = i + 1; j < splitedTargetString.length; j++) {
                    ...
                    if (j === splitedTargetString.length - 1)
                        lastIndex = j - 1;
                }
